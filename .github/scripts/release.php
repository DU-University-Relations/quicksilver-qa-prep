...
$packagist_api_token = getenv('PACKAGIST_API_TOKEN');
...
try {
    ...
    if (in_array($github['repository'], $packages['packageNames'])) {
        ...
        $packagist_url = "https://packagist.org/api/update-package?username={$packagist_username}&apiToken={$packagist_api_token}";
        $data = json_encode(['repository' => ['url' => "https://packagist.org/packages/{$github['repository']}"]]);
    } else {
        // Package doesn't exist, create new package
        $packagist_url = "https://packagist.org/api/create-package?username={$packagist_username}&apiToken={$packagist_api_token}";
